<script lang="ts">
	...
	function startResize(e: PointerEvent) {
		...
		function onMove(ev: PointerEvent) {
			height = Math.min(MAX_HEIGHT, Math.max(MIN_HEIGHT, startHeight + (ev.clientY - startY)));
		}
		function onUp() {
	...
	$effect(() => {
		segments;
		waitingForInput;
		// Scroll within the console container only, not the page
		const container = scrollTarget?.parentElement;
		if (container) {
...
<div class="flex flex-col">
	<div class="rounded-lg border border-zinc-800 bg-zinc-950 overflow-hidden">
		...
		<div class="overflow-y-auto p-3 font-mono text-sm" style="height: {height}px" role="log" aria-live="polite">
			{#if !hasContent}
	...
	<!-- Resize handle — outside overflow-hidden so it's always grabbable -->
	<!-- svelte-ignore a11y_no_static_element_interactions -->
	<div
		class="h-2 cursor-ns-resize flex items-center justify-center group"
